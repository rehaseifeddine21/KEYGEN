...
def generate_mobile_scale_key(device_id):
    if not device_id or len(device_id) < 3:
        return 'En attente ID Scale...'
...
class KeygenApp(App):
    ...
    def update_calculation(self, *args):
        try:
            ...
            elif self.active_source == 'mobile_scale' or self.current_mode == 'mobile_scale':
                key = generate_mobile_scale_key(hwid)
                self.result_title = 'Clé Mobile (Scale):'
